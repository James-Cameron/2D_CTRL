...
public class James2dCTRL_v06 : MonoBehaviour
{
    /*
    ...
    Version6: (2020-08-14) Fixed bug where wall grab to the left only worked if player continuously holds left.  Corrected an issue
                           with the aerial drift code.  Added precautionary groundcheck in Fall().  Added Glide() function.
    ...
    */
    ...
    void Drift()
    {
        // MIGHT WANT TO INCREASE THE AIR ACCEL
    ...
    void Glide()
    {
        ...
        if (isGliding == true && velocity.y < 0)
        {
            Physics2D.gravity = new Vector2(0f, -1 * glideFactor);

            glideTimer = Time.time;
        }

        if ((Time.time - glideTimer) > maxGlideTime)
        {
            isGliding = false;
        }
